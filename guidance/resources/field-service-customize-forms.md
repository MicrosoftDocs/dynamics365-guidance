...
1. View and familiarize yourself with the default fields, and determine which ones you can use for your business.
...
1. Enter work order details. These details include the account, work order type, location, products, services, service tasks, and any other information that is important.
1. Schedule the work order to one or more resources (frontline workers). The system automatically changes the status to *Scheduled*.
1. A frontline worker views the scheduled work order on their mobile app and travels to the customer location to perform the required work. The worker changes the system status to *In Progress*. The frontline worker updates information such as the time when they arrived on-site, the service tasks that they completed, and the services and products to bill.
1. The frontline worker completes the work order. The worker changes the system status to *Completed*.
1. The back-office manager or dispatcher reviews the completed work order and verifies that the work was completed and the necessary data was captured. If everything is finished, they change the system status to *Posted*.
...
Make a list of default work order fields that are relevant to your business process. Make another list of unneeded fields. Remember to add [the critical fields](#critical-fields) to the list of needed fields, because they're required.
...
Some scenarios call for a custom work order form. Here are some of the top reasons why you might need to create a custom work order form:
...
- **You can make more edits.** You can make more changes to a custom form, so that it fits your unique work order process.
...
You can also start from a copy of the default form if you made changes to that form.
...
Hide fields that you don't need, unless they're [critical fields](#critical-fields).
...
1. Select **Component** on the toolbar. In the **Components** pane on the left, select **1-column tab**, **2-column tab**, or **3-column tab**.
...
To see how factors such as browsers, network performance, and data queries affect app and form performance, [run performance insights](/power-apps/maker/common/performance-insights-overview).
