...
The *Vendor self-service for technician management* solution is a guide for how to build and configure a flexible foundation that helps organizations efficiently manage external vendors and contractors with Dynamics 365 Field Service. You can customize the solution to meet your specific business needs. It automates the process of setting up vendor resources, managing their characteristics, and handling their access to the system. These resources can then sign in to the Field Service mobile app to perform field work.

> [!NOTE]
> This solution is a sample and starting point for implementers to create vendor self-service management capabilities tailored to their business needs. It's a recommendation for how to achieve that goal and comes as-is. System implementers can extend the solution as needed.
...
- Create an Microsoft Entra ID user account.
...
With the *Vendor self-service for technician management* solution, external vendors or in-house resource management can do the following tasks:

- Automated invitations to [Microsoft Entra External ID in business-to-business (B2B) collaboration](/entra/external-id/what-is-b2b) so that Entra ID users are automatically created with the relevant licenses, they're added to the security groups, and new contractors are automatically created as [bookable resources](/dynamics365/field-service/set-up-bookable-resources).
- Manage contractors easily through a simple UI in the **Contact** table/forms, with real-time automated synchronization with the settings for bookable resources, skills, and work hours.
...
We provide this solution and code as-is for Dynamics 365 implementors. Always test in a nonproduction environment. You can adjust and extend this solution as needed. All code is open source, and the Dynamics 365 solution file is unmanaged to give you full flexibility to adjust, add, or remove features.

## Prerequisites

To install and use this solution, you must have the following prerequisites:
...
The following diagram illustrates how the various components, including Dynamics 365, Azure Functions, and Microsoft Entra External IDs in B2B collaboration interact with each other.

:::image type="content" source="media/fs-vendor-self-service-for-technician-management/vendor-self-serve-architecture.svg" alt-text="Screenshot of the architecture with the components are described in the Components section.":::

This solution uses a lightweight model-driven app that gives vendor administrators (the vendor representative responsible for managing their resources in your system) permission through [Microsoft Entra External ID in B2B collaboration](/entra/external-id/b2b-fundamentals). Within this app, vendor admins use the **Contact** table as a portal to provision, manage, and deprovision their resources as needed. This solution has automation to handle the back-end tasks required for these scenarios based on interactions with the Contact record.

> [!NOTE]
> Contact records that vendor admins access are just a **window** into the Bookable resource for the given contractor. The Bookable resource has a resource type of **User** and links to a System User record.
...
1. Azure Functions (.NET)
   - **InviteVendorResource**: HTTP Function to process the Microsoft Entra External ID B2B invites for new contractor setup.
...
1. Install the provided Dynamics 365 solution in your environment.
1. Configure the necessary environment variables for both.
...
1. After you publish your Azure Functions, go to the Function App in the Azure portal. In the Settings area on the left, select **Environment variables** and create three new environment variables with the following names:
   - `ClientId`
   - `ClientSecret`
   - `TenantId`

1. Populate the `ClientId`, `ClientSecret`, and `TenantId` environment variables with values from a service principal. Ensure the service principal has permissions to send Microsoft Entra B2B invites, create users, add users to Microsoft Entra security groups, and apply or remove Microsoft 365 licenses. Refer to the [permissions to add](/entra/identity-platform/quickstart-configure-app-access-web-apis#application-permission-to-microsoft-graph) in the following screenshot or in the [Prerequisites](#prerequisites) section.
...
1. Turn on Azure Functions and make them available for invocation from Power Automate flows in your tenant.
...
1. Since it's an unmanaged solution, publish all customizations after importing the solution.  

    > [!IMPORTANT]
    > Make sure to turn on the three Power Automate flows.
...
Vendor administrators use this solution to manage their resources. The vendor administrator must set up as a licensed user in your Dynamics 365 environment. Invite this user through Microsoft Entra External ID B2B, like the contractors. They can sign in to your environment by using their email address and the password they create during the invitation process. Vendor administrators must have licenses that allow them to interact with Contact and Bookable resource records through a custom model-driven app.
...
1. Open the new Contact form by selecting **+New** in the command bar.
1. Fill in resource information.
   - Enter at least the **First Name**, **Last Name**, and **Email** fields.
   - Include the **Technician Address** field if you need location-aware scheduling.
   - Save the record. If you fill in the **Technician Address** field, the system geocodes the address and populates the coordinates in the **Address 1: Latitude** and **Address 1: Longitude** fields.

   > [!NOTE]
   > Changes to the **Name** and **Email** fields don't sync to the Microsoft Entra user record after creating the user. Make these changes manually in the Microsoft Entra admin center if needed.
...
1. Set fields on the **Field Service** tab. These fields sync directly to the Bookable resource record when created but you can also change them after creating the Bookable resource.
1. Add **Technician Characteristics** to the resource. These characteristics apply to the Bookable resource as Bookable resource characteristics. When creating a new Technician Characteristic record, you don't need to populate the **Rating** field.

   > [!NOTE]
   > The **Work Hours** tab doesn't display the Work Hours control until the system creates the Bookable resource.

1. Toggle the **Setup Technician** flag to **Yes** to start provisioning automations for the resource.
1. The system invokes a Power Automate flow that calls Azure Functions to:
   - Send an [invitation through Microsoft Entra B2B](/entra/external-id/invitation-email-elements) to the email on the Contact and create a [Guest-type user](/entra/external-id/user-properties) in Microsoft Entra.

      > [!NOTE]
      > The Microsoft Entra B2B invitation doesn't need to be accepted before continuing. The invitation can be [accepted at any time](/entra/external-id/user-properties#before-invitation-redemption) without impeding resource provisioning and setup.

   - Assign your designated license to the new user.
   - Add the new user to your designated security group.
   - Create a Bookable resource when it creates the user record and link it to the Contact through a hidden Bookable resource lookup field on the Contact form.

      > [!NOTE]
      > The [user creation process](/power-platform/admin/create-users?tabs=new#add-users-to-dataverse) for Dynamics 365 is asynchronous and might take some time after license assignment.

1. After the user [accepts their invitation](/entra/external-id/user-properties#after-invitation-redemption), they can sign in to the Field Service mobile app to perform fieldwork.

   > [!NOTE]
   > The username that the resource uses for signing in to the Field Service mobile app depends on whether the email address is a work or school account. If the email address is a work or school account, the user resource signs in to the app with their email address. If the email address isn't a work or school account (for example, Gmail, Yahoo, or Outlook.com), the user resource must use their fully qualified username to sign in. This username follows the schema `emailaddress#EXT#@domain`, where the `@` in the email address is replaced with an underscore (`_`). You can view this username in the Microsoft 365 admin portal or the Microsoft Entra admin center.
...
- If the address changes from the initial values set on the Contact form, manually change the geo code of the record by selecting **Geo Code** in the Contact's command bar.

   > [!NOTE]
   > To ensure that the vendor admin's address entry on the Contact form takes precedence, the solution inherently ignores changes to the Latitude and Longitude fields on the System User record. A plugin registered on updates to the System User table enforces this behavior. This behavior prevents address changes in Microsoft Entra for the user from affecting the System User record if a matching contact exists for that user (not for all system users), based on email address. You can disable this plugin step if you don't need this functionality.
...
- **To deprovision a resource**: Deactivate the Contact record. This action unassigns the license from the respective Microsoft Entra user and deactivates the system user shortly after.
- **To reprovision an existing resource**: Reactivate the Contact record. This action reassigns the license to the respective Microsoft Entra user and reactivates the system user.

> [!NOTE]
> Removing a license from a user triggers an asynchronous process to deactivate the user in Dataverse, which might take some time to reflect. The same process applies to reactivating the user after reapplying the license.

## Related resources

- [Set up bookable resources - Field Service](/dynamics365/field-service/set-up-bookable-resources)
- [Create users - Power Platform](/power-platform/admin/create-users?tabs=new#add-users-to-dataverse)  
- [Provision an Azure Function App](/azure/azure-functions/functions-create-function-app-portal?pivots=programming-language-csharp)  
